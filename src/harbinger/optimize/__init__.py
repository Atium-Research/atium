"""
Portfolio optimization primitives.

This module provides:
- Covariance matrix construction from factor models
- Mean-variance optimization with cvxpy
- Dynamic gamma targeting for active risk control
"""

import numpy as np
import polars as pl
import cvxpy as cp


# =============================================================================
# Covariance Matrix
# =============================================================================

def build_factor_covariance(
    factor_loadings: pl.DataFrame,
    factor_covariances: pl.DataFrame,
    idio_vol: pl.DataFrame,
    date=None,
) -> tuple[np.ndarray, list[str]]:
    """
    Build covariance matrix using factor model: Σ = B @ F @ B' + D
    
    Args:
        factor_loadings: DataFrame[ticker, date, factor, loading]
        factor_covariances: DataFrame[date, factor_1, factor_2, covariance]
        idio_vol: DataFrame[ticker, date, idio_vol]
        date: Optional date to filter to
    
    Returns:
        (covariance_matrix, tickers) - matrix is n x n, tickers defines ordering
    """
    # Filter to specific date if provided
    if date is not None:
        if "date" in factor_loadings.columns:
            factor_loadings = factor_loadings.filter(pl.col("date") == date)
        if "date" in factor_covariances.columns:
            factor_covariances = factor_covariances.filter(pl.col("date") == date)
        if "date" in idio_vol.columns:
            idio_vol = idio_vol.filter(pl.col("date") == date)

    # Get common tickers
    tickers_with_loadings = set(factor_loadings["ticker"].unique().to_list())
    tickers_with_idio = set(idio_vol["ticker"].unique().to_list())
    tickers = sorted(tickers_with_loadings & tickers_with_idio)

    if not tickers:
        return np.array([[]]), []

    factor_loadings = factor_loadings.filter(pl.col("ticker").is_in(tickers))
    idio_vol = idio_vol.filter(pl.col("ticker").is_in(tickers))

    factors = sorted(factor_loadings["factor"].unique().to_list())
    n_assets = len(tickers)
    n_factors = len(factors)

    # Build B matrix (factor loadings)
    B = np.zeros((n_assets, n_factors))
    loadings_pivot = factor_loadings.pivot(
        on="factor", index="ticker", values="loading"
    ).sort("ticker")
    for i, factor in enumerate(factors):
        if factor in loadings_pivot.columns:
            B[:, i] = loadings_pivot[factor].fill_null(0).to_numpy()

    # Build F matrix (factor covariance)
    F = np.zeros((n_factors, n_factors))
    for row in factor_covariances.iter_rows(named=True):
        f1, f2, cov = row["factor_1"], row["factor_2"], row["covariance"]
        if f1 in factors and f2 in factors:
            i, j = factors.index(f1), factors.index(f2)
            F[i, j] = cov
            F[j, i] = cov

    # Build D matrix (idiosyncratic variance)
    idio_sorted = idio_vol.sort("ticker")
    idio_var = idio_sorted["idio_vol"].fill_null(0.02).to_numpy() ** 2
    D = np.diag(idio_var)

    # Σ = B @ F @ B' + D + regularization
    cov_matrix = B @ F @ B.T + D
    cov_matrix = cov_matrix + np.eye(n_assets) * 1e-6

    return cov_matrix, tickers


# =============================================================================
# Risk Calculation
# =============================================================================

def calculate_active_risk(
    weights: np.ndarray,
    benchmark_weights: np.ndarray,
    cov_matrix: np.ndarray,
    annualize: bool = True,
) -> float:
    """
    Calculate active risk (tracking error).
    
    active_risk = sqrt(active_weights' @ Σ @ active_weights) * sqrt(252)
    
    Args:
        weights: Portfolio weights (n,)
        benchmark_weights: Benchmark weights (n,)
        cov_matrix: Covariance matrix (n x n)
        annualize: Whether to annualize (multiply by sqrt(252))
    
    Returns:
        Active risk as a float
    """
    active_weights = weights - benchmark_weights
    variance = active_weights @ cov_matrix @ active_weights
    vol = np.sqrt(max(variance, 0))
    if annualize:
        vol *= np.sqrt(252)
    return float(vol)


# =============================================================================
# Mean-Variance Optimization
# =============================================================================

def _solve_mvo(
    alphas: np.ndarray,
    cov_matrix: np.ndarray,
    gamma: float,
) -> np.ndarray | None:
    """
    Solve MVO using cvxpy.
    
    Maximize: α' @ w - (γ/2) * w' @ Σ @ w
    Subject to: sum(w) = 1, w >= 0
    
    Args:
        alphas: Expected returns (n,)
        cov_matrix: Covariance matrix (n x n)
        gamma: Risk aversion parameter
    
    Returns:
        Optimal weights or None if solver fails
    """
    n = len(alphas)
    w = cp.Variable(n)

    objective = cp.Maximize(
        w @ alphas - 0.5 * gamma * cp.quad_form(w, cov_matrix)
    )

    constraints = [
        cp.sum(w) == 1,
        w >= 0,
    ]

    problem = cp.Problem(objective, constraints)
    problem.solve()

    return w.value


def mean_variance_optimize(
    expected_returns: dict[str, float],
    cov_matrix: np.ndarray,
    tickers: list[str],
    gamma: float = 1.0,
) -> dict[str, float]:
    """
    Mean-variance optimization with fixed gamma.
    
    Args:
        expected_returns: Dict mapping ticker -> expected return (alpha)
        cov_matrix: Covariance matrix (n x n)
        tickers: List of tickers (defines ordering)
        gamma: Risk aversion parameter
    
    Returns:
        Dict mapping ticker -> optimal weight
    """
    n = len(tickers)
    if n == 0:
        return {}

    alphas = np.array([expected_returns.get(t, 0) for t in tickers])
    weights = _solve_mvo(alphas, cov_matrix, gamma)

    if weights is None:
        weights = np.ones(n) / n

    return dict(zip(tickers, weights))


# =============================================================================
# Dynamic Gamma (Target Active Risk)
# =============================================================================

def _predict_gamma(data: list[tuple[float, float]], target_risk: float) -> float:
    """
    Predict gamma to achieve target active risk.
    
    Uses linear model based on relationship: active_risk ≈ M / (2 * gamma)
    
    Args:
        data: List of (gamma, active_risk) from previous iterations
        target_risk: Target active risk
    
    Returns:
        Predicted gamma value
    """
    data_arr = np.array(data)
    gammas = data_arr[:, 0]
    sigmas = data_arr[:, 1]

    # Fit M via least squares: X = 1/(2*gamma), y = sigma
    X = 1.0 / (2.0 * gammas)
    M = np.dot(X, sigmas) / np.dot(X, X)

    # gamma = M / (2 * target_risk)
    return M / (2.0 * target_risk)


def mean_variance_dynamic(
    expected_returns: dict[str, float],
    cov_matrix: np.ndarray,
    tickers: list[str],
    benchmark_weights: dict[str, float],
    target_active_risk: float = 0.05,
    max_iterations: int = 5,
    tolerance: float = 0.005,
) -> tuple[dict[str, float], float, float]:
    """
    MVO with dynamic gamma to achieve target active risk.
    
    Iteratively adjusts gamma (risk aversion) until the resulting portfolio
    achieves the target active risk. This is the approach from nt-backtester.
    
    Args:
        expected_returns: Dict mapping ticker -> expected return
        cov_matrix: Covariance matrix (n x n)
        tickers: List of tickers (defines ordering)
        benchmark_weights: Dict mapping ticker -> benchmark weight
        target_active_risk: Target annualized active risk (e.g., 0.05 = 5%)
        max_iterations: Max iterations to find optimal gamma
        tolerance: Acceptable error in active risk
    
    Returns:
        (weights_dict, final_gamma, achieved_active_risk)
    """
    n = len(tickers)
    if n == 0:
        return {}, 0.0, 0.0

    alphas = np.array([expected_returns.get(t, 0) for t in tickers])
    bench = np.array([benchmark_weights.get(t, 0) for t in tickers])

    # Normalize benchmark weights
    if bench.sum() > 0:
        bench = bench / bench.sum()
    else:
        bench = np.ones(n) / n

    gamma = None
    data: list[tuple[float, float]] = []
    weights = np.ones(n) / n
    active_risk = float("inf")

    for _ in range(max_iterations):
        # First iteration: start with gamma=100, then predict
        if gamma is None:
            gamma = 100.0
        else:
            gamma = _predict_gamma(data, target_active_risk)
            gamma = max(1.0, min(gamma, 10000.0))

        # Solve MVO
        result = _solve_mvo(alphas, cov_matrix, gamma)
        if result is not None:
            weights = result

        # Calculate achieved active risk
        active_risk = calculate_active_risk(weights, bench, cov_matrix)
        data.append((gamma, active_risk))

        # Check convergence
        if abs(active_risk - target_active_risk) <= tolerance:
            break

    return dict(zip(tickers, weights)), gamma, active_risk
