...
class MinVarianceWithTargetActiveRisk(Objective):
    """Minimum variance portfolio calibrated to hit a target active risk.

    Solves: min w'Σw + μ(w-b)'Σ(w-b)

    Iteratively fits the relationship AR = C / (1+μ) to find the μ
    that produces the desired annualized tracking error vs. the benchmark.
    ...
    def build(
        ...
        """Calibrate mu to match the target active risk, then return the objective."""
        covariance_matrix: np.ndarray = kwargs.get('covariance_matrix')
        benchmark_weights: np.ndarray = kwargs.get('benchmark_weights')
        optimizer_constraints = kwargs.get('constraints', [])
        ...
        while abs(active_risk - self.target_active_risk) > error:
            if mu is None:
                ...
                mu = self._predict_mu(data, self.target_active_risk)

            w = cp.Variable(n_assets)
            obj = cp.Minimize(
                cp.quad_form(w, covariance_matrix)
                + mu * cp.quad_form(w - benchmark_weights, covariance_matrix)
            )
            constraints = [c.build(w, **kwargs) for c in optimizer_constraints]
            prob = cp.Problem(obj, constraints)
            prob.solve()
            ...
            data.append((mu, active_risk))

            if len(data) == 1 and active_risk <= self.target_active_risk:
                break
        ...
        return cp.Minimize(
            cp.quad_form(weights, covariance_matrix)
            + mu * cp.quad_form(active_weights, covariance_matrix)
        )

    @staticmethod
    def _predict_mu(data: list[tuple[float, float]], active_risk: float) -> float:
        """Predict the mu that will produce the target active risk.

        Uses a linear model fit on 1/(1+mu) vs. observed active risk
        from prior iterations: AR = C / (1 + mu).
        """
        data_np = np.array(data)
        mu_vals = data_np[:, 0]
        ar_vals = data_np[:, 1]
        X = 1.0 / (1.0 + mu_vals)
        C = np.dot(X, ar_vals) / np.dot(X, X)
        predicted_mu = C / active_risk - 1.0
        return max(predicted_mu, 0.0)
